  ...
  export default {
    ...
    created() {
      if(this.dataSign==''){
        ...
        this.myData = new Date(this.dataSign);
      }
    },
    methods: {
      ...
      getList: function (date, chooseDay, isChosedDay = true) {
        ...
        var datatimechoose=this.dataSign==''?this.signFormat(timenowList):this.dataSign
        for (var i = 0; i < this.getDaysInOneMonth(date); i++) {
          ...
          if (
            datatimechoose == timeList && !chooseDay
          ) {
            array = array.concat({
              id: i + 1,
              date: nowTime,
